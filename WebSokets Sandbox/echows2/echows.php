...
function sselect() {
    ...
    $sselect_timer = $async('sselect', 1000);
    
    $time2 = microtime(1) - $time;
    
    if ($time2 > $counter) {
      echo ' --- LOG: Prev: ' . round(($duration * 1000), 2) . ' ms. Duration: ' . round($time2, 2) . ' sec. Memory: ' . round((memory_get_usage(true) / 1000000), 2) . ' MB --- <br>';
      $counter++;
    }
    //$time = microtime(1);
}
...
while (true) {
    ...
    if ($tick === true) {
        ...
        //fwrite($connects[0], encode('---------------'));
        //fwrite($connects[0], encode('PING PING PING'));
        //fwrite($connects[0], encode('PING PING PING'));
        //fwrite($connects[0], encode('PING PING PING'));
        //fwrite($connects[0], encode('---------------'));
        ...
        if (false === stream_select($read, $write, $except, 0)) {//ожидаем сокеты доступные для чтения (без таймаута)
            break;
        }
        ...
        if (in_array($socket, $read)) {//есть новое соединение то обязательно делаем handshake
          ...
          echo "Connect = " . $connect.", info = " . $info . "<br>";          
          //echo "info<br />";     
          //var_dump($info); 
